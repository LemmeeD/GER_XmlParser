...
namespace GER_XmlParser.entities
{
    public class MyTree<T> where T : class
    {
        ...
        public static MyTree<XmlNode> ComputeFromModelFindRef(XmlNodeList nodesFound, XmlNode nodeRootToBeExcluded)
        {
            MyTreeNode<XmlNode> myRoot = new MyTreeNode<XmlNode>(nodeRootToBeExcluded);
            ...
            foreach (XmlNode node in nodesFound)
            {
                ...
                if (firstOfChain == null) firstOfChain = myRoot.AddChild(first);
                myCursor = firstOfChain;
                foreach (XmlNode cursor in parentsChainUntilNodeTobeExcluded.Skip(1))
                {
                    MyTreeNode<XmlNode> newMyCursor = myCursor.AddChild(cursor);
